...
def minimax(board, depth, ply, maximisingPlayer, startTime, timeLimit, alpha=-999999, beta=999999, allowNull=True):
    ...
    for move in moves:
        ...
        if not pv:
            ...
            if (score > alpha) and (score < beta): # If score better than bot's current best guaranteed score and also better than opponents best guaratneed score
                score = minimax(board, depth - 1, ply + 1, not maximisingPlayer, startTime, timeLimit, alpha, beta)
    ...
    if not legalMovesFound:
        if board.kingCheck(currentColour):
            return (-99999 + ply) if maximisingPlayer else (99999 - ply)
        else:
            return 0
...
def searchMovesAtDepth(board, moves, depth, ply, alpha, beta, maximisingPlayer, botColour, startTime, timeLimit):
    ...
    for move in moves:
        ...
        if not pv: # Assumes first move searched is previous best move (principal variation)
            ...
            if (score > alpha) and (score < beta): # Score must be better than current best guaranteed path and opponents best guaranteed path otherwise there is either no point or could be shut down instantly by playing beta
